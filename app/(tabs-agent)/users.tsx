    ...
    async function load() {
        try {
            const [data, statsData] = await Promise.all([
                getUsers({ role: selectedRole || undefined, search: search || undefined }),
                getUsersStats(),
            ]);
            setItems(data.items);
            setStats(statsData);
                ...
                { text: "Annuler", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            setItems(prev =>
                                prev.map(u => (u._id === user._id ? { ...u, isActive: r.isActive } : u))
                            );
                        } catch (e: any) {
    ...
    return (
        ...
        >
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 12 }}>
                ...
                >
                    <Text style={[styles.chipText, !selectedRole && styles.chipTextActive]}>Tous</Text>
                </Pressable>
                {ROLES.map(r => (
                    <Pressable
                        key={r}
                    ...
                    >
                        <Text style={[styles.chipText, selectedRole === r && styles.chipTextActive]}>
                            {ROLE_LABELS[r]}
                        </Text>
                    </Pressable>
                ))}
